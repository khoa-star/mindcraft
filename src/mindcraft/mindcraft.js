...
export async function createAgent(settings) {
    ...
    try {
        ...
        // ===== AUTO RECONNECT PATCH =====

        agentProcess.process?.on('exit', (code, signal) => {

            console.log(
                `Agent ${agent_name} exited (code=${code}, signal=${signal}). Restarting in 5s...`
            );
            ...
            setTimeout(() => {
                ...
                try {
                    ...
                    if (agent_processes[agent_name]) {

                        agent_processes[agent_name].forceRestart();
